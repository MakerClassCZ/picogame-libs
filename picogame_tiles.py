...
class TileFlags:
    def __init__(self, flags=None, tile_px=8):
        """flags: {tile_index: bitfield} dict, or a list/bytes indexed by tile index. tile_px =
        tile size for the pixel helpers (default 8)."""
        if isinstance(flags, (list, tuple, bytes, bytearray)):
            self.f = {i: v for i, v in enumerate(flags) if v}
        else:
    ...
    def set(self, tile, bit, value=True):
        v = self.f.get(tile, 0)
        self.f[tile] = (v | (1 << bit)) if value else (v & ~(1 << bit))
