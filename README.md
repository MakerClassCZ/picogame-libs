...
| `picogame_launcher` | Engine-native list+preview launcher (320x240 baseline) - the shell that picks and starts a game on a device that holds several. |
...
| `picogame_music` | Play PICO-8 tracker music through `synthio` - the import path for chiptune written in PICO-8's tracker. Four channels cost roughly 3 ms/frame on RP2040. |
